...
def run_sql(
    ...
    if payload.mode == "document_extract":
        ...
        if _is_csv_document(payload.document_path):
            extraction = _csv_extraction(payload.text, payload.document_path)
        ...
        try:
            ...
            raw_json = provider.complete(
                _document_ingest_user_prompt(payload.text, payload.document_path),
        ...
        if not json_text:
            raise HTTPException(502, "LLM returned empty JSON")
        try:
            extraction = DocumentExtraction.model_validate_json(json_text)
        except Exception as e:
            raise HTTPException(502, detail={"json": json_text[:4000], "error": str(e)}) from e
    ...
    if not payload.question:
        raise HTTPException(422, "question is required when mode=query")
